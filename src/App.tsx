...
const AppContent = () => {
  ...
  // Check if current path is a patient route (patient dashboard and related pages)
  const currentPath = window.location.pathname;
  const patientRoutes = ['/patient-dashboard', '/patient-register', '/patient/'];
  const isPatientRoute = patientRoutes.some(route => currentPath.startsWith(route));

  // Check if current path is an auth route (patient-signup is public, login should always be accessible)
  const authRoutes = ['/signup', '/signup-full'];
  const isAuthRoute = authRoutes.includes(currentPath);

  // Check if current path is a public route (doctor discovery, homepage, login, and patient signup)
  const publicRoutes = ['/', '/login', '/patient-signup', '/doctors', '/doctor/', '/book/', '/appointment/confirm/'];
  const isPublicRoute = publicRoutes.some(route =>
    route === '/' ? currentPath === '/' : currentPath.startsWith(route)
  );

  // Allow patient routes and public routes without staff authentication
