...
export default function DoctorMeeting() {
  ...
  const loadDoctorSettings = async () => {
    ...
    try {
      ...
      if (data) {
        // Cast to any to access zoom fields that may not be in generated types
        const doctorData = data as any;
        setDoctor(doctorData);
        setZoomLink(doctorData.zoom_meeting_link || '');
        setZoomPassword(doctorData.zoom_password || '');
        setZoomMeetingId(doctorData.zoom_meeting_id || '');
        setInstructions(
          doctorData.zoom_instructions ||
            'Please join 5 minutes before your appointment time.'
        );
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Use any type to update zoom fields that may not be in generated types
