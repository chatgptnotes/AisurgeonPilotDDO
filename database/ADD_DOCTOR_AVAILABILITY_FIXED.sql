-- Add Doctor Availability - FIXED VERSION
-- Now includes tenant_id to fix the NOT NULL constraint error
...
BEGIN
  ...
  LOOP
    ...
    FOR v_day IN 1..5 LOOP
      ...
      IF NOT v_exists THEN
        -- Create new availability WITH tenant_id
  ...
  RAISE NOTICE '==============================================';
  RAISE NOTICE 'SUCCESS! All doctors now have availability.';
  RAISE NOTICE 'Created Mon-Fri 9AM-5PM schedules for all doctors.';
  RAISE NOTICE '==============================================';
END $$;
...
SELECT
  ...
  MIN(da.start_time)::TEXT as opens_at,
  MAX(da.end_time)::TEXT as closes_at
FROM doctors d
...
-- Show detailed availability for verification
SELECT
  d.full_name,
  CASE da.day_of_week
    WHEN 1 THEN 'Monday'
